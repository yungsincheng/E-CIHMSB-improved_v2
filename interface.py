                    ...
                    if verify_img:
                        ...
                        if 'verify_img_result' in st.session_state and st.session_state.verify_img_result:
                            ...
                            if vr.get('same_size'):
                                mse = vr['mse']
                                if mse == 0:
                                    st.markdown('<div style="font-size: 16px; font-weight: bold; color: #4f7343;">完全一致！</div>', unsafe_allow_html=True)
                                else:
                                    st.markdown('<div style="font-size: 16px; font-weight: bold; color: #C62828;">不一致！</div>', unsafe_allow_html=True)
                            else:
                                st.markdown(f'<div style="font-size: 16px; color: #C62828;">尺寸不同，無法比較<br>原始：{vr["orig_size"][0]}×{vr["orig_size"][1]} vs 提取：{vr["ext_size"][0]}×{vr["ext_size"][1]}</div>', unsafe_allow_html=True)
