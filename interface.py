...
if st.session_state.current_mode is None:
    ...
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    # ----- 用 JS 把 Streamlit 按鈕移到螢幕外（視覺上隱藏）-----
    components.html("""
<script>
...
""", height=0)
        ...
        with col3:
            ...
            if step2_done:
                ...
                if images:
                    ...
                    if capacity_ok:
                        usage_text = f"使用率：{usage:.1f}%"
                    else:
                        over_ratio = secret_bits_needed / capacity
                        usage_text = f"容量不足！（需要 {over_ratio:.1f} 倍空間）"
        ...
        # ===== 開始嵌入按鈕 =====
        all_done = step1_done and step2_done and st.session_state.get('embed_image_id') and st.session_state.get('embed_capacity_ok', False)
                    ...
                    if verify_img:
                        ...
                        if 'verify_img_result' in st.session_state and st.session_state.verify_img_result:
                            ...
                            if vr.get('same_size'):
                                mse = vr['mse']
                                if mse == 0:
                                    st.markdown('<p style="color: #4f7343;">完全一致！</div>', unsafe_allow_html=True)
                                else:
                                    st.markdown('<p style="color: #C62828;">不一致！</div>', unsafe_allow_html=True)
                            else:
                                st.markdown(f'<p style="color: #C62828;">尺寸不同，無法比較<br>原始：{vr["orig_size"][0]}×{vr["orig_size"][1]} vs 提取：{vr["ext_size"][0]}×{vr["ext_size"][1]}</div>', unsafe_allow_html=True)
